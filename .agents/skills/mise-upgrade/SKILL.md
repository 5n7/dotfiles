...
Update every tool managed by mise to its latest available version and record the new versions in `.config/mise/config.toml`.

## Process

1. Run `mise outdated --bump` to list tools that have newer versions available. If all tools are current, report that and stop.
2. Run `mise upgrade --bump` to install the latest versions and update the pinned versions in `.config/mise/config.toml`.
3. From the dotfiles root, run `git diff -- .config/mise/config.toml` to show the exact version bumps.
...
- `--bump` is required to update the version strings in `.config/mise/config.toml`; without it, `mise upgrade` only installs the latest patch within the pinned range.
