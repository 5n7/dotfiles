...
- Manage branches as worktrees with [git-wt](https://github.com/k1LoW/git-wt); when starting work on a new branch, first check whether the current directory is already inside a worktree (e.g. `git rev-parse --git-dir` contains `.git/worktrees/`). If so, reuse it; otherwise run `git wt <branch>`.
