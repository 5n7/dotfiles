...
- Always start herdr-orchestrated agents in that kind's native yolo / skip-approvals mode so they do not stall on permission UIs (`blocked`). Pass the flag after `--`, for example `herdr agent start <name> --kind claude --pane <id> -- --dangerously-skip-permissions`. Known flags: `claude` `--dangerously-skip-permissions`; `codex` `--dangerously-bypass-approvals-and-sandbox`; `grok` `--always-approve`. Other kinds: the equivalent yolo, bypass-permissions, or always-approve flag. If the kind has none, start it normally.
